...
\frame[label=FREXAMPLE]{
  ...
  \begin{centering}
    \tnote{say something less apologetic!}
...
\frame[label=FREXAMPLE2]{
  ...
  \tnote{CHUCK THE TEXTUAL SET DEFINITIONS. ONLY DIAGRAMS.}

  \begin{block}{} 
  \begin{itemize}[<uncover@+->]
  \item A set of \alert{$n$ students} arrive for a summer course, say
    { $\{\xPa,\; \xPi,\; \xSn,\; \xWo,\; \xVi,\; \xLi,\; \xCh,\;
    \xSa,\; \xFr,\; \xSc,\; \xLu\}$, $n= 11$}
  \tnote{a venn diagram of just the universe}

  \item They form \alert{$m$ study groups}, say {$\{\xLLL,
      \xGGG, \xBBB, \xTTT\}$, $m = 4$}
  \tnote{a venn diagram of the grouping}
...
\frame[label=FREXAMPLE3]{
  ...
  \item A student may be in more than one study group but \alert{will be in
    at least one}, say \\
 ...
 \end{tabular}
  ...
  \item There are \alert{$n$ single occupancy apartments} in \residenceblock.
  \tnote{image of infinite loop}

  \item Streets connecting them \alert{do not form loops.}
  \end{itemize}
  \end{block}
...
\frame{ \frametitle{Tree Path Labeling of Set Systems}
  \framesubtitle{The combinatorial problem terminology}

  \tnote{SPLIT THIS INTO FOUR SLIDES WITH CORRESPONDING PREVIOUS
    IMAGES REPEATED} 

  \begin{block}{Terminology}

    \begin{itemize}[<uncover@+->]
    \item The set of study groups $\{\xLLL,
      \xGGG, \xBBB, \xTTT\}$ $\rightarrow$
      \alert{\sc Hypergraph}
    \item {\residenceblock} block $\rightarrow$ \alert{\sc
        Target tree}
    \item Study group path allocation $\rightarrow$
      \alert{\sc Tree Path Labeling (TPL)}
    \item The apartment allocation $\rightarrow$ \alert{\sc Path
...
%  \note<1>{}
  ...
  \begin{block}<1->{1}
    {\alert{Characterization} of a {\em feasible TPL} and finding
      the certificate for feasibility i.e. {\em hypergraph
        isomorphism}}
  \end{block}
  \note[item]<1>{Characterization -- hypergraph + target tree + TPL\\ 
      (1) is it feasible? \\
      (2)what is the isomorphism?}

  \begin{block}<2>{2}
    {\alert{Computation} of a {\em feasible TPL} if any}
  \end{block}
