...
%\includeonlyframes{SLIDENOW2}    % Use when drafting for fast
...
\frame [label=SLIDENOW2]{ 
  ...
  \begin{block}{}
    ...
    \item \alert{Super marginal sets} are assigned an arbitrary ray not
      considered yet.
    \item Sets that overlap with the super marginal set are considered
      just like in ICPIA algorithm \cite{nsnrs09} until a path
...
\frame [label=SLIDENOW2]{ 
  \frametitle{Theory of prime submatrices}%
  \begin{block}{}
    \begin{itemize}[<uncover@+->]
