...
\tnote{ The main themes of the thesis that must not be left out:\\
  connection of COP to *SET SYSTEMS* and thus to general
  data bases. cite:nsnrs09\\
  *GRAPH ISOMORPHSIM* by canonization cite:kklv10\\
  *CERTIFICATE FOR A PROBLEM* cite:mcc04\\
...
\section{Consecutive-ones Property Testing - a Brief Survey} %{Background}
...
\tnote{ADD NEW SECTIONS - MATCHING WHAT'S IN INTRO CHAPTER}
...
\tnote{UPDATE TO FINAL VERSION. BELOW IS OLDER VERSION WITH
  INCOMPLETE SEC 4.}
As we saw earlier, tree path labeling problem of path hypergraphs is a
generalization of consecutive-ones property testing.

\remove{The problem of COP testing can be easily seen as a simple
  constraint satisfaction problem involving a system of sets from a
  universe (analogous to hypergraph). Every column of the binary
  matrix can be converted into a set of integers which are the indices
  of the rows with |un|s in that column. When observed in this
  context, if the matrix has the COP, a reordering of its rows will
  result in sets that have only consecutive integers. In other words,
  the sets after reordering are intervals. Indeed the problem now
  becomes finding interval assignments to the given set system
  cite|nsnrs09| with a single permutation of the universe (set of row
  indices) which permutes each set to its interval. The result in
  cite|nsnrs09| characterizes interval assignments to the sets which
  can be obtained from a single permutation of the rows.  They show
  that for each set, the cardinality of the interval assigned to it
  must be same as the cardinality of the set, and the intersection
  cardinality of any two sets must be same as the interesction
  cardinality of the corresponding intervals.  While this is naturally
  a necessary condition, cite|nsnrs09| shows this is indeed
  sufficient.  Such an interval assignment is called an Intersection
  Cardinality Preserving Interval Assignment (ICPIA).  Finally, the
  idea of decomposing a given 0-1 matrix into prime matrices to check
  for COP is adopted from cite|wlh02| to test if an ICPIA exists for a
  given set system.}\tnote{REPEATITION from intro chapter}
...
graph.}{the following text could be in Further Study/Conclusion}  A similar connection to a subclass of chordal graphs and a
...
\tnote{give problem definition etc}
