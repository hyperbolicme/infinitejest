...
\begin{definition}[Hypergraph isomorphism induced path labeling]
  ...
  Then there is an \textbf{induced path labeling} % denoted by $\cl_\phi:
%   \cF \rightarrow \cP$ 
...
\begin{proof}
  ...
  $P_{two}$ be the elements of $S$ and $P$ which are in one {\em
    other} set and path, respectively i.e. $S_{two} = S \cap (\cup_{S_i
    \ne S} S_i)$ and $P_{two}$ is defined analogously. It may be noted
  that $S = \supt({\cF'})$ and $P = \supt({\cF'}^{\cl'})$.
  From Lemma~\ref{lem:icpia} it follows that there is a bijection
  $\phi$ from $S$ to $P$ such that for each $i$, the image
  of $S_i \cap S$ under $\phi$ is $P_i \cap P$ (definition of
  feasibility).  Moreover, this means that in $\phi$,
...
\begin{proof}\thesisspacing
  ...
  function because the target tree is non-empty.%\qed
\end{proof}
...
After handling leaf 2, \filteri proceeds to handle leaf 3 which is
...
In Section~\ref{sec:motive} it is described how \COP is a special case
...
% CHAIN PARTITIONS
% each leaf to a degree >= two vertex...
% without include the degree >= two vertex
% that is one chain  
% do tis for all leaf
...
Note that in this example the matrix has been explicitly labeled with
...
\begin{algorithm}[htb]
  ...
  \begin{algorithmic}[\lndisplay]
    ...
    \FOR {each prime submatrix $M_x$ other than $mub(\bP_i)$ in $\bP_i$}%
...
% Indeed, in this case we encounter a restatement of matrices with the
% COP. 
...
\begin{theoremsansproof}[{\cite[Th.~6.1]{mcc04}, \cite[Corrected by
    Th.~2.6, Proof p. 44--47]{d08phd}}, Theorem~\ref{th:mccbipartite}] 
...
\begin{theorem}[{\cite[Th.~4.7]{kklv10}}]
  \label{th:canonlabel}
