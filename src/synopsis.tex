...
give some basic gyan about the problem itself? or screw it move to
section directly

\section{Illustration of the problem}
\label{sec:problem}

\def \LLL {{\em ``{\bf L}aughing with Kafka''}} %{Consider the {\bf
                                %L}obster} 
\def \GGG {{\em ``{\bf G}irl with Curious Hair''}} % {``{\bf G}od Bless You,
                                % Mr. Franzen''} 
\def \BBB {{\em ``{\bf B}rief Interviews with Hideous Men''}} %{{\bf B}room
                                %of the System} 
\def \TTT {{\em ``{\bf T}his is Water''}} %{``{\bf T}ennis, Trigonometry,
...
% enroll at the
{\em Wallace Studies Institute} for a liberal arts programme.  As part
...
allocation to students. \tnote{use tables}
\tnote{use other font for elements of sets}
\begin{enumerate}[]
...
allocation to individual students. One possible allocation of
exclusive routes to study groups is illustrated in
Fig~\ref{fig:streetmap+path}.

\begin{figure}[h]
  % \centering
  (a) \includegraphics[scale=0.3]{../img/a1.pdf}\label{fig:streetmap}
  (b) \includegraphics[scale=0.3]{../img/a2.pdf}\label{fig:streetmap+path}
  \caption{(a) {\residenceblock} street map (b) Street map with study
    group routes allocated. Routes are color coded as follows: red for
    \textcolor{red}{\em \LLL} group, blue for \textcolor{blue}{\em
      \GGG} group, orange for \textcolor{YellowOrange}{\em \BBB} group,
    green for \textcolor{green}{\em \TTT} group}
\end{figure}
...
\begin{figure}[h]
  ...
  \caption{(i) matrix with COP (ii) without COP}
...
\tnote[a]{define ``COP permutation''}

\section{Background}
Different aspects of COP were studied as part of this thesis. They are
described in the following subsections.

\subsection{Consecutive Ones Testing}
...
invented; PQ trees \cite{bl76}, variations of PQ trees
\cite{mm09,wlh01,wlh02,mcc04}, ICPIA \cite{nsnrs09} are some of them.

\cite{bl76} showed for the first time that COP testing (COT) is indeed
polynomial time solvable by describing the construction of a data
structure called PQ trees. This paper draws on the close relationship
of COP matrices to interval graphs. A PQ tree of a matrix is one that
stores all row (column) permutations of the matrix that give the COP
orders (there could be multiple) of the matrix. This is constructed using an elaborate procedure in the linear
time algorithm described in \cite{bl76}.\tnote[a]{verify if it is
  linear}. Since this result, several improvements have been made to
the procedural complexity of the algorithm itself and also derivative
data structures that are easier to construct. PQR trees is one such
data structure \cite{mm96}, \cite{mpt97}\tnote{also, mpt97 $==$
  elaborate version of mm96}.  \cite{tm03} describes an improved
version of PQR trees. \tnote[a]{improvement in terms of
  what?}. \cite{h01a} describes the simpler algorithm for
COT. \tnote[a]{This result first apprearted in proc ISAAC 92} Hsu also
invented PC trees \cite{h01b} which is claimed to be much easier to
implement. \cite{nsnrs09} describes a characterization of consecutive
ones property solely based on the cardinality properties of the set representations
of the columns (rows); every column (row) is equivalent to a set that
has the row (column) indices of the rows (columns) that have one
entries in this column (row). This is interesting and relevant,
especially to this thesis because it simplifies COT to a great
degree. \tnote[a]{because it reduces the search space for the solution
  from set of all permutations of rows to set of all-- get this
  figured out!}
...
machinery of this algorithm \annote[a] {is related to the weighted
  betweenness problem addressed } {in what way??} in \cite{co97}.
\tnote[a]{expand on the COP order graph creation and it having to be
  bipartite for M to have COP. and thus an odd cycle being an evidence
  of no COP.}

\tnote[a]{THESIS: where should this go?: \cite{jlm97} (application of
  PQ trees in graphics).}

% \noindent
To elaborate on what was mentioned earlier about \cite{nsnrs09}, the
problem of COT can be seen as a constraint satisfaction problem
involving a system of sets from a universe as follows. Every column of
the binary matrix can be converted into a set of integers which are
the indices of rows with $1$s in that column. It is apparent that if
the matrix has COP, then constructing such sets after applying the COP
permutation to the matrix will result in all sets with consecutive
integers. In other words, after application of COP reordering, the
sets are intervals. Indeed the problem now becomes finding interval
assignments to a given set system such that there exists a permutation
of the universe (set of row indices) which converts each set to its
assigned interval. The result in \cite{nsnrs09} characterizes interval
...
of the interval assigned to it must be same as the cardinality of the
set, and the intersection cardinality of any two sets must be same as
the intersection cardinality of the corresponding intervals.  While
this is obviously a necessary condition, this result shows this is
also sufficient.  \cite{nsnrs09} calls such an interval assignment an
Intersection Cardinality Preserving Interval Assignment (ICPIA).  This
paper generalizes the idea from \cite{wlh02} of decomposing a given
binary matrix into prime matrices for COT and describes an algorithm
to test if an ICPIA exists for a given set system.

The literature also has results on parallel COT algorithms --
\cite{as95} \cite{bs03} \tnote[a]{is this parallel?}
\cite{cy91}\tnote[a]{is this parallel?}

\tnote[a]{THESIS: add helly's theorem citation
  19XXdgk-Hellystheorem-Danzer-Gruenbaum-Klee}

\subsection{Matrix modification to attain COP or ``almost'' COP}
\tnote[a]{THESIS: results and survey from \cite{d08phd} Chapters 2 and
  3}
...
\tnote[a]{check if it is the other way around}.\\
...
bipartite graphs \tnote[a]{THESIS: check this up. give details.} which
...
interest to find out the smallest\tnote[a]{define ``smallest''}
forbidden substructure. \cite{d08phd} discusses an exact algorithm
with time complexity \annote[a]{parameterized by the number of ones in
  the matrix}{check this.}.  
...
complete. \tnote[a]{check if b75 deals with COP col or COP row. also
  is it any submatrix with k less than r rows or submatrix must have
  all columns?}. \cite{hgxx} \tnote[a]{get year XX} corrects an error of the abridged proof
...
algorithms for this problem. \tnote[a]{THESIS: elaborate - what are
  the results?}.
...
a reduction to the longest path problem, \tnote[a]{or is it a survey
  of another result?  check.} that finding a Tucker's forbidden
submatrix of at least $k$
rows is NP complete. \tnote[a]{how is this different from booth's 75 result??} \tnote{[tag:appoximation,bitFlipping,matrixBreaking]}\\

\tnote[a]{THESIS: where should this go? \cite{tz03} (approx submatrix with COP sparse matrices)}


%\subsection{Further extensions of the COP problem}

\cite{jkckv04} \tnote{[tag:storage]} discusses the use of matrices
...
introduced. \tnote[a]{THESIS? what does this mean? -- COP can be used as a tool to reorder $0.5T \le runs(M) \le T$}

The optimization version of the $k$-run problem, i.e. minimization of
number of blocks of ones is proven to be NP complete by
\cite{kou77}. \tnote[a]{THESIS: what is the reduction?}

\tnote[a]{THESIS: 
-- {\em other problems similar to COP}
  \begin{enumerate}
  \item \cite{ckl96} (ILP, circ ones, one drop)
  \item \cite{th99} (generalization of COP - minimax, biotonic column) Tucker
  \end{enumerate}
}
...
\tnote[a]{THESIS: peo exists iff chordal. lexicographic BFS
  [tag:chordalGraph]} 
...
interval graph if and only if \tnote[a]{verify from paper the
...
in polytime $O(m+n)$ \tnote[a]{citation?!!}.  \cite{fg65} uses these
results to give the first polynomial time algorithm for
COT. \tnote[a]{THESIS: check. how do they use it?}  A bipartite graph
is convex (on $R$) if and only if its half adjacency matrix has COP on
rows.  The results in \cite{bl62} on COT are based on the result that
interval graphs are AT-free chordal graphs. \tnote[a]{the latter being
  Tucker's?}

\tnote[a]{THESIS: -- How COP (and TPL) relates to interval graph
  isomorphism (and path graph isomorphism) move/summarize what is
  already in Motivation section.}

\tnote[a]{THESIS: TBD survey -- see blue notes under Graph
  Isomorphism. namely citations in \cite{aas93}}

\tnote[a]{THESIS: -- -- a brief on heirarchy of chordal graphs, path
  graphs, interval graphs, peo, clique tree etc. and results.}

\tnote[a]{THESIS: -- interval graphs are incomparability graphs. see
  Golumbic. [tag:classification]}

\tnote[a]{THESIS: finding min length hole in bipartite graph is
  polynomial time [Sec 3.3.2 in \cite{d08phd}]}

\tnote[a]{THESIS: theorem 2.2 in \cite{d08phd} - $G$ is union of
  v.d. caterpillars iff $M$ has COP in rows. $M$ is edge vertex
  incidence matrix $(*,2)$ of $G$}


\subsection{Logspace complexity of graph canonization using COP}

Canonization is an important tool in graph isomorphism. Invention of a
...
algorithm. \tnote[a]{confirm the statement of claim}.
...
counting) definable \tnote[a]{elucidate} and \cite{l10} showed that
this implies that it captures PTIME. \tnote[a]{elucidate} This result
along with that of undirected graph reachability being in logspace
\cite{r08}, \cite{kklv10} proved their logspace result.\\ 

\tnote[a]{THESIS: -- -- the interesting survey in conclusion section
  of kklv10}

\tnote[a]{THESIS:
{\bf COP helping some problems with hardness}
having COP in the input structure makes some problems less hard than
in a general input.

-- min set cover problem is known to be NP hard. so is min hitting
...
\tnote[a]{THESIS: \cite{d08phd} Chapter 5.}

\tnote[a]{THESIS: what are \cite{d08phd} results}


\tnote[a]{THESIS: -- Rectangle stabbing \tnote[a]{see \cite{d08phd} Chapter 6?}}


\tnote[a]{THESIS: -- {\em \cite{dgnw06} (min membership hitting set + COP)}}
...
that maintain the characterization\tnote[a]{REWORD}.  Further, the
...
for our question.  \remove[a]{Indeed, our work provides a sufficient
...
\annote[a]{it is unlikely that we can solve the problem of finding
  feasible path labeling $\cl$ for a given $\cF$ and }{WHY?} tree $T$.
\annote[a]{It is definitely interesting to classify the kinds of trees
  and hypergraphs for which feasible path labelings can be
}{ELABORATE.} found efficiently.  \annote[a]{These results would form
...
%\item 
In the Section 4, \annote[a]{we initiate an exploration of}{WTF?}
finding feasible path labeling of set systems in a special kind of
tree which we call the $k$-subdivided star.  \annote[a]{This question
...
elements.  \annote[a]{In spite of this restricted case we consider, we
