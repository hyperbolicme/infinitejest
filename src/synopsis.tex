...
\tnote{give some basic gyan about the problem itself? or screw it move to
section directly}
...
tree\tnote{\footnote{Allowing any tree in this example could be seen
    as a scenario where there are parallel classes. A node falling in
    the path between two other nodes would mean that the corresponding
    is scheduled between the other two.}}) is called an interval
assignment problem.

\tnote{TBD: BEGIN REVIEW FROM BELOW}
...
\tnote[a]{THESIS: UNCOMMENT} \tnote{include the rest of the sections as
  the organization of the doc}
...
\cite{mm96}, \cite{mpt98}\tnote{also, mpt98 $==$ elaborate version of
  mm96}.  \cite{tm05} describes an improved version of PQR
trees. \tnote[a]{improvement in terms of what?}\cite{wlh02} describes
the simpler algorithm for COT. \tnote[a]{This result first apprearted
  in proc ISAAC 92} Hsu also invented PC trees \cite{wlh01} which is
...
thesis because it simplifies COT to a great degree. \tnote[a]{because
  it reduces the search space for the solution from set of all
  permutations of rows to set of all-- get this figured out!}
...
\cite{ly91}\tnote[a]{is this parallel?}
...
\tnote[a]{THESIS: where should this go? \cite{tz04} (approx submatrix with COP sparse matrices)}
...
\cite{k77}. \tnote[a]{THESIS: what is the reduction?}

\tnote[a]{THESIS: 
-- {\em other problems similar to COP}
  \begin{enumerate}
  \item \cite{ckl96} (ILP, circ ones, one drop)
  \item \cite{th98} (generalization of COP - minimax, biotonic column) Tucker
  \end{enumerate}
}
