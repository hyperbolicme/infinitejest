...
\def \xBBB {\mathbb{W}}
\def \xTTT {\mathbb{F}}
% {{\em ``{\bf B}rief Interviews with Hideous Men''}} 
% {{\em ``The String {\bf T}heory''}} 
% {{\em ``[{\bf W}]Rhetoric and the Math Melodrama''}}
% {{\em ``{\bf F}ate, Time, and Language: An Essay on Free Will''}}
% {{\em ``Everything and More: A Compact History of In{\bf F}inity''}} 
% {{\em ``Broom of {\bf T}he System''}} 
% {{\em ``Consider the Lobster''}}
...
graphs are AT-free in \cite{at72}\tnote{check}. The first linear time
...
a test for planarity\tnote{check check check. both interval
...
adjacent to those nodes that represent the columns that have ones in
...
a submatrix $M'$ with all columns\tnote{check if b75 deals with COP
  col or COP row. also is it any submatrix with k less than r rows or
  submatrix must have all columns?} but a maximum cardinality subset
...
parameterized algorithms for this problem\tnote[TH]{elaborate - what
  are the results?}.
...
of ones is proven to be NP complete by cite:k77}\tnote{are these two
...
preservation makes a tree path labeling \footnote{A
  {\em tree path labeling} $\cl$ is a bijection of paths from the
  target tree $T$ to the hyperedges in given hypergraph $\cF$.} (TPL) feasible, which is a
generalization of the COP problem. This problem is
defined as follows.
...
We give a necessary and sufficient condition (called ICPPL)\tnote{say a few
  words about icppl} for {\FTPL} to output in
 ...
 \vspace{2mm}
  ...
  \begin{tabular}[t]{l|l}
    ...
    \begin{minipage}[t]{\probdefwidth}
      ...
      $S \in \cF$ is of cardinality atmost $k+2$ and a {\kstar} $T$.\\
    \end{minipage}\\
    {\tt Question} &
    \begin{minipage}[t]{\probdefwidth}
      Does there exist a set of paths $\cP$ from $T$ and a bijection
...
of $\cF$ from subtrees of $T$.\tnote[TH]{The connection
of TPL to graph isomorphism will be made later in the document}
%\end{enumerate}
...
\tnote[TH]{(1) How COP (and TPL) relates to interval graph isomorphism
  (and path graph isomorphism) move/summarize what is already in
  Motivation section.(2) TBD survey -- see blue notes under Graph
  Isomorphism. namely citations in cite:aas93 (3) a brief on heirarchy
  of chordal graphs, path graphs, interval graphs, peo, clique tree
  etc. and results. (4) interval graphs are incomparability
  graphs. see Golumbic. [tag:classification] (5) finding min length
  hole in bipartite graph is polynomial time [Sec 3.3.2 in
  cite:d08phd] (6) theorem 2.2 in cite:d08phd - $G$ is union of
