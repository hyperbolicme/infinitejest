...
\begin{definition}
  [Matrices that define graphs {\cite[Def.~2.4]{d08phd}}]  
...
\begin{definition}[Graphs that relate to
...
\begin{definition}[\Cop for set systems]%
...
\begin{definition}[Orthogonal sets \cite{n89, mm96,
...
% When a \COP test algorithm reports negative, it is not easy to verify
% this. We already saw that \cite{at72} gives forbidden substructures
% for \COP. \cite{v82} shows that the maximization problem of finding a forbidden submatrix with at
...
% problem. For most cases, the minimization problem is more helpful.
% Section~\ref{sec:forbiddensubalgo} gave a polynomial
% algorithm for finding minimum size forbidden submatrix. 
...
Note that this algorithm has a factor $\Delta$ in the time complexity
...
\begin{definition}[{Incompatibility graph \cite[Def.~6.1]{mcc04}}]
