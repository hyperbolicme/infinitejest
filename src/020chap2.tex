...
\begin{table}[htbp]
  \centering
  {
   ...
   \begin{tabular}[h]{lll}
     ...
     \hline
     % 
     1899 & \parbox[t]{\tabparboxw}{ First mention of \COP} & \cite{p59}  \\ 
...
\begin{definition}%
  ...
  of order $n \times m$ % , $r_i, i \in [n]$ be the rows in $M$
...
Thus the idea of \cop naturally maps to set sytems as well\endnote{As
  seen in Section~\ref{theCourseScheduleExample}} as
...
\temptext{ Certifying algorithm (Generalized PQ trees) [McC04] -- Move
  this to section on certifying algo.
