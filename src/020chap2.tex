...
\begin{definition}{\emph{Graphs that relate to
 ...
 \item $G$ is a \emph{circular-arc graph} if its vertices can be
    ...
    \iff their corresponding arcs overlap.\tnote[pressing]{how is
      CO/ROP related?}
  \item \label{def::convexbi} $H$ is \emph{convex bipartite on columns (rows)} if its half adjacency matrix
    has \COP on rows (columns).
...
\item \label{q:nocopcert} If algorithm for (\ref{q:testcop}) returns \textbf{false}, can a
  certificate for this be computed?
\end{enumerate}
...
which is not surprising due to Definition~\ref{def:graphwithcop}~\ref{def::convexbi}. 

\temptext{
  \begin{enumerate}[-I-]
  \item PQ trees. Define. Concise flowchart of algorithm? [BL76]\\
    The construction of PQ trees in \cite{bl76} draws on the close
    relationship of matrices with COP to interval graphs. A PQ tree of
    a matrix is one that stores all row (column) permutations of the
    matrix that give the COP orders (there could be multiple orders of
    rows or columns) of the matrix. This is constructed using an
    elaborate linear time procedure and is also a test for
    planarity\tnote{check check check. both interval graph and
      planarity in this paper?}.
