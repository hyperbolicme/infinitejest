...
\begin{definition}[\em Orthogonal sets \cite{n89, mm96,
  ...
  \item $\cF$ is called {\em complete}\footnote{\cite{mcc04} calls
      this a {\em weakly partitive family}. \label{mcc2}} if the
...
\item \label{th::it2}$\overline{\cF} = consec(\alpha)$ for
  some permutation $\alpha$ on $U$ ($consec(\alpha)$ is a linear order)
...
are no efficient algorithms to find these structures in a matrix. In
...
$\cF$.  \cite{mcc04} creates an incompatibility graph with vertices
