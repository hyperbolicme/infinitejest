...
\begin{definition}[Binary matrix]
  ...
  element, i.e. element at $i$th row and $j$th column. $M$ is a \textbf{binary
  matrix} if each of its element is 0 or \un.  In other words, for all
...
\begin{definition}[Permutation]
  ...
  $\lambda$ may be written as a sequence $x_{i_1}x_{i_2}\ldots x_{i_n}$,
  where $i_j \in [n]$ to mean that $\lambda(x_j)= x_{i_j}$.
...
\begin{definition}[\Cop]
  ...
  \begin{itemize}
  \item \textbf{\Cop}
  \item \textbf{Strong \cop}
  \item \textbf{\Cop order} or \cop permutation
  \end{itemize}
\end{definition}

\figcopmatrix

\begin{definition}[\Crop]
...
\begin{definition}[Partial order]
  ...
  \item \textbf{maximal upper bound}  $X_m \in X$ is a \textbf{maximal upper bound} of $X$
  if $\nexists X_q \in X$ such that $X_m \preccurlyeq X_q$.  A maximal
  upper bound on $X$ is denoted by $mub(X)$.
...
\begin{definition}[Graph, Tree]
  \label{def:graphtree}  
  \tnote[dire]{take from doms}
\end{definition}
...
\begin{definition}[Interval graph, Path graph]
  ...
  \begin{itemize}
  \item A graph $G$ is an \textbf{interval graph} if there a set of intervals
    $\cI$ \stt $G$ is isomorphic to the intersection graph of
    $\cI$. \[G \cong \bI(\cI)\]
  \item A graph $G$ is a \textbf{path graph} if there exists a tree $T$ and a
    set of paths from $T$, $\cP$ \stt $G$ is isomorphic to the
    intersection graph of $\cP$. \[G \cong \bI(\cP)\]
  \end{itemize}
\end{definition}
Moreover, a \textbf{chordal graph} is characterized as an intersection
graph of subtrees of a tree.
